...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        ...
        const allowedTypes = ['profile_image', 'admin_image'];
        if (!allowedTypes.includes(type)) {
            return cb(new Error("Invalid 'type' parameter"), null);
        }
...
imageUploadRouter.use('/uploadimage/:type/:filename', (req, res) => {
    const { type, filename } = req.params;
    const filePath = path.join(uploadDirectory, type, filename);
